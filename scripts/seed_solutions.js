...
INITIAL_SOLUTIONS['quan-ly-nuoc-thong-minh'] = INITIAL_SOLUTIONS['dieu-hoa-trung-tam-vrv-chiller'];
INITIAL_SOLUTIONS['nong-nghiep-chinh-xac'] = INITIAL_SOLUTIONS['giai-phap-gach-op-lat-du-an'];
INITIAL_SOLUTIONS['quan-trac-thuy-san'] = INITIAL_SOLUTIONS['dieu-hoa-cuc-bo-thuong-mai'];

async function main() {
    console.log('Seeding trilingual site_solutions into system_settings...');
    const query = `
    ...
    `;
    await pool.query(query, ['site_solutions', JSON.stringify(INITIAL_SOLUTIONS)]);
    console.log('Successfully seeded trilingual site_solutions into database!');
    await pool.end();
}

main().catch((e) => {
    console.error('Error seeding solutions:', e);
    process.exit(1);
});
